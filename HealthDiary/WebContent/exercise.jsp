			...
			<main class="h-full pb-16 overflow-y-auto">
				<div class="container grid px-6 mx-auto">
					...
					<label class="block text-sm"> <span
						class="text-gray-700 dark:text-gray-400">운동명</span>
						<form action="findexercise.do" id="submitfrom">
							<input id="exname"
								class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
								placeholder="스쿼트" name="exercisename" />
						</form>
						<button
							class="px-3 py-1 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-md active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple"
							type="submit" form="submitfrom">검색</button>
					</label>
							...
							<table class="w-full whitespace-no-wrap">
								<thead>
									<tr
										...
										<th class="px-4 py-3">유사한 운동 리스트</th>
									</tr>
								</thead>
								<tbody
									class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
									<c:forEach items="${nvo}" var="item">
											...
											<td class="px-4 py-3">
												<div class="flex items-center text-sm">
													...
													<div>
														...
														<a href="searchexercise.do?exname=${ item.getName() }">${ item.getName() }</a>
					...
					<div class="w-full overflow-hidden rounded-lg shadow-xs">
										...
										<tr class="text-gray-700 dark:text-gray-400">
											<td class="px-4 py-3">
												<div class="flex items-center text-sm">
													...
													<div>
														<p class="font-semibold">
														<a href="searchexercise.do?exname=${list.ename }">${list.ename }</a>
														</p>
													</div>
												</div>
											</td>
						...
						<div
							...
							<span class="flex items-center col-span-3"> Showing
								${(page.page-1)*10+1}-${(page.page-1)*10+10} of
								${total.exercistListTotal} </span> <span class="col-span-2"></span>
							<!-- Pagination -->
									...
									<ul class="inline-flex items-center">
										<li>
											<button
												...
												aria-label="Previous">
												<c:if test="${page.prev }">
													<a href="findexercise.do?page=${(page.startPage)-1}"><svg
                            ...
                            <path
										...
										<c:forEach begin="${page.startPage }" end="${page.endPage }"
											...
											<c:if test="${page.page != num }">
												<li><a href="findexercise.do?page=${num}">
														<button
										...
										<li>
											<button
												...
												aria-label="Next">
												<c:if test="${page.next }">
													<a href="findexercise.do?page=${(page.endPage)+1}"> <svg
